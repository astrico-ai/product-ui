...
// Use a separate storage key for mining dashboards
const STORAGE_KEY = 'mining_dashboards';

// Helper functions to get and save mining dashboards
const getMiningDashboards = () => {
  const dashboardsJson = localStorage.getItem(STORAGE_KEY);
  return dashboardsJson ? JSON.parse(dashboardsJson) : [];
};

const saveMiningDashboards = (dashboards) => {
  localStorage.setItem(STORAGE_KEY, JSON.stringify(dashboards));
};

// Mining dashboard management functions
const listMiningDashboards = () => {
  return getMiningDashboards();
};

const createMiningDashboard = (data) => {
  const dashboards = getMiningDashboards();
  const newDashboard = {
...
const deleteMiningDashboard = (id) => {
  const dashboards = getMiningDashboards();
  const filteredDashboards = dashboards.filter(d => d.id !== id);
  saveMiningDashboards(filteredDashboards);
};

export default function MiningDashboardList() {
  ...
  useEffect(() => {
    const loadDashboards = () => {
      const storedDashboards = listMiningDashboards();
  ...
  const handleCreateDashboard = (data) => {
    const newDashboard = createMiningDashboard({
      name: data.name,
    ...
    navigate(`/mining/dashboard/${newDashboard.id}`);
  };

  const handleDeleteDashboard = (id) => {
    deleteMiningDashboard(id);
  ...
  return (
    <MainLayout>
      <div className="max-w-[1400px] mx-auto px-8 py-6">
        ...
        <p className="text-gray-500 mb-8">Manage and organize your mining analytics dashboards</p>
              ...
              {sortedDashboards.length === 0 ? (
                ...
                sortedDashboards.map((dashboard) => (
                  <TableRow key={dashboard.id} className="hover:bg-gray-50">
                    <TableCell>
                      <a 
                        href={`/mining/dashboard/${dashboard.id}`}
                    ...
                    <TableCell className="text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem onClick={() => navigate(`/mining/dashboard/${dashboard.id}`)}>
                            Edit
                          </DropdownMenuItem>
